...
<div class="main-banner header-text" id="top" style=" background: linear-gradient(to bottom, #dffbf0 30%, #ffffff 93%);">
    ...
    <div class="container home-con">
        <div class="half1">
            <h3 class="mt-5" style="color:rgb(47,155,92);font-size: 60px;">Blog Details</h3>
        </div>
    </div>
...
<div class="container">
    ...
    <input style="margin-top: 100px" type="button" value="Delete" class="button_uv1">
  </a> --}}



    <div class="row justify-content-around" style="margin-bottom: 50px; margin-top: 70px ">
      <div class="col-4">
        ...
        @if (Auth::User()->id == $blog->user_id)
          <div class="button-container">
            <a class="update_button" href="{{route('blog.edit', ['user'=>Auth::User()->username, 'blog_id'=>$blog->id, 'slug'=>Str::slug($blog->title)])}}">
            ...
            <a href="#" onclick="confirmDelete({{ $blog->id }})" class="delete_button">
      ...
      <div class="col-6">
        <h2> {{$blog->title}} </h2>
        <h6>#{{$username}}<small class="text-body-secondary" style="margin-left:10px">Created at {{ $blog->created_at->format('h:i A | d F, Y') }}</small></h6>
        ...
        @php
          $min = $blog->updated_at->diffInMinutes(now());
          $hour = floor($min/60);
          $day = floor($hour/24);
        @endphp
    ...
    <div class="card-body" style="background:#d3f5df9d;margin-bottom: 70px; border-radius: 10px;">
      ...
      <p class="card-text" style="margin-top: 30px">{!! nl2br($blog->details) !!}</p>
    </div>
...
<script>
  function confirmDelete(blogId) {
      if (confirm("Are you sure you want to delete this blog post?")) {
          window.location.href = "{{ route('blog.delete', ['id' => ':id']) }}".replace(':id', blogId);
      }
  }
</script>
